...
Did you have wished to be able to create more "dynamic" documents with AsciiDoc?
...
Another concept ot jBake is to first parse and store all content in a local database and only then apply the templates.
...
The list might grow quite quickly and ss soon as you want to display such a (long) list in another way, you need to reformat _all_ entries.

In case of the measurements of your IoT device, it is even more obvious - you just want to type the measuremts by hand into an AsciiDoc table.
...
You already _build_ your docs, so why not convert some json formatted data into AsciiDoc?
...
//itarate over data and output list of talks
def asciidoc = ""
...
You can then include the resulting file in any doc.
...
The main difference is now that we inject code within <% ... %> into our document instead the other way around.
The code shown could be embedded in a larger document and since this is part of the document, you don't have to start an additinal build step before your build your docs.
